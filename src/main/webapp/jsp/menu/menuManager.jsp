...
<body class="gray-bg black-bg-gmtx">
		...
		<div class="ibox float-e-margins">
			<div class="ibox-title">
				<div class="row">
					...
					<div class="col-sm-1">
						<button class="btn btn-gmtx-define1" onclick="delModule()" id="btn_del">
							删除
						</button>
					</div>
				</div>
									...
									<form method="post" class="form-horizontal" id="addform">
										<div class="form-group">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" id="pname_add" disabled/>
												<input type="text" style="display:none" id="parentCode" name="parentCode"/>
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" name="moduleName" id="moduleName" />
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" name="modulePath" id="modulePath" />
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
										...
										<div class="form-group">
											...
											<div class="col-sm-8 ">
												<input type="text" value="" class="form-control" name="sortNumber" id="sortNumber" />
											</div>
										</div>
								...
								<div class="row">
									<form method="post" class="form-horizontal" id="upform">
										<div class="form-group">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" id="pname_edit" disabled/>
												<input type="text" style="display:none" id="parentCode_edit" name="parentCode"/>
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" name="moduleName" id="moduleName_edit" />
												<input type="text" style="display:none" name="moduleCode" id="moduleCode_edit" />
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
											...
											<div class="col-sm-8 controls">
												<input type="text" value="" class="form-control" name="modulePath" id="modulePath_edit" />
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-3 control-label">
											...
											<div class="col-sm-8" id ="isLeaf_edit">
												<div class="radio radio-inline">
			                                        <input type="radio" value="1" name="isLeaf"  id="isLeaf1_edit">
			                                        <label for="isLeaf1_edit"> 是 </label>
			                                    </div>
			                                    <div class="radio radio-inline">
			                                        <input type="radio" value="0" name="isLeaf" id="isLeaf0_edit" >
			                                        <label for="isLeaf0_edit"> 否 </label>
			                                    </div>
											</div>
										</div>
										<div class="form-group">
											...
											<div class="col-sm-8 ">
												<input type="text" value="" class="form-control" name="sortNumber" id="sortNumber_edit" />
											</div>
										</div>
										<div class="control-group">
											<div class="controls">
	...
	<!-- jQuery -->
		...
		<script type="text/javascript" src="/${appName}/commons/jslib/js-gmxt-define/ztreeTool.js"></script>
		<script type="text/javascript" src="/${appName}/commons/jslib/hplus/js/plugins/jasny/jasny-bootstrap.min.js"></script>
		...
		/**share---*/
			...
			function loadProTree(){
			        $.ajax({
					url:'/${appName}/manager/moduleController/getTreeList',
					type:'post',
					async:'true',
					cache:false,
					data:{},
					...
					success: function(data){
						$.fn.zTree.init($("#protree"), setting, data);
					}
				    });
			    }
			//节点点击事件
			function zTreeOnClick(event, treeId, treeNode) {
					...
					if(isSelected("protree"))
					{
						...
						{
							$('#upwin').show();
							loadSingleData(getSelected("protree").moduleCode);
						}else
						{
							$("#addwin").hide();
							$("#upwin").hide();
			...
			{
		 		$.ajax({
       				url:'/${appName}/manager/moduleController/getSingleData',
       				dataType:'json',
       				data:{moduleCode:id},
       				type:'post',
       				success:function(data){
			      		...
			      		$("#isLeaf_edit>input[name='isLeaf']:checked").prop('checked',false);
			      		$("#isLeaf"+data.moduleData.isLeaf+"_edit").prop('checked',true);
				      	...
				      	if(data.moduleData.moduleCode=='0'){
				      		...
				      		$("#isLeaf_edit input[name='isLeaf']").attr('disabled',true);
			...
			function showAdd(){
				if(isSelected("protree")){
					var node_isLeaf = jsonToObj(getSelected("protree").isLeaf);
					if(!isEmpty(node_isLeaf)&&node_isLeaf=='1'){
						swal('系统提示','该节点为叶子节点!','warning');
						return;
					}
					$("#pname_add").val(getSelected("protree").moduleName);
					$("#parentCode").val(getSelected("protree").moduleCode);
					$('#upwin').hide();
					$('#addwin').show();
				}else{
			...
			function addModule(){
				...
				$.ajax({
					url:'/${appName}/manager/moduleController/addModule',
					type:'post',
					async:'true',
					cache:false,
					data:{moduleName:moduleName,modulePath:modulePath,parentCode:parentCode,isLeaf:isLeaf,sortNumber:sortNumber},
					dataType:'json',
					success: function(data){
							  if(data.exist=='existName'){
				                  ...
				                  swal({
							           ...
							           $("#moduleName").val('');
							      	   $("#modulePath").val('');
							      	   $("#parentCode").val('');
							      	   $("#isLeaf_add input[name='isLeaf']:checked").attr('checked',false);
							      	   $("#sortNumber").val('');
							           $("#addwin").modal('hide');
									   loadProTree();
							         });
			...
			/**-删除---*/
			  function delModule(){
				...
				ids = "'"+selects[0].moduleCode+"'";
		        for(var i = 1;i<selects.length;i++){
			        if(selects[i].moduleCode==0){
			        	swal('系统提示','请勿删除根节点！','warning');
			        	return;
			        }
		    		ids+=",'"+selects[i].moduleCode+"'";
		        }
			  	swal({
		            title: "您确定要删除吗",
		            text: "删除后将无法恢复，请谨慎操作！",
		            ...
		            if (isConfirm) {
		                $.ajax({
							url:'/${appName}/manager/moduleController/delModule',
							type:'post',
							async:'true',
							cache:false,
			  ...
			  function upModule(){
			  		...
			  		$.ajax({
						url:'/${appName}/manager/moduleController/upModule',
						type:'post',
						async:'true',
						cache:false,
						data:{moduleCode:moduleCode,moduleName:moduleName,modulePath:modulePath,parentCode:parentCode,isLeaf:isLeaf,sortNumber:sortNumber},
						dataType:'json',
						success: function(data){
								if(data.exist=='existName'){
							  	  swal({
			  ...
			  function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
				var selfId=treeNodes[0].moduleCode;
				var parentId=targetNode.moduleCode;
				
				$.ajax({
					       		url:'/${appName}/manager/moduleController/changeJoin',
					       		dataType:'json',
					       		data:{selfId:selfId,parentId:parentId},
					       		type:'post',
