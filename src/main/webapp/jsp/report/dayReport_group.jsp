...
<head>
    ...
    <script>
        ...
        function getPie(time){
            /*获取种类*/
            $.ajax({
                url: '/${appName}/manager/reportController/getAllDayByMonth_group',
                type: 'post',
                async: 'true',
                cache: false,
                data: {time:time},
                ...
                success: function (monthdata) {
                    if (!isNull(monthdata)) {
                        /*获取数据*/
                        $.ajax({
                            url: '/${appName}/manager/reportController/getDayMoneyByMonth_group',
                            type: 'post',
                            async: 'true',
                            cache: false,
                            data: {time:time},
                            ...
                            success: function (moneydata) {
                                if (!isNull(moneydata)) {
                                    ...
                                    option = {
                                        title: {      //标题组件
                                            text: time+"月每日消费情况",
                                            subtext: "当月总计"+sum.toFixed(2)+"元   单位:元",
                                             x: 'center'
                                        },
                                        tooltip : {
                                            trigger: 'axis'
