...
<html>
	...
	$(function(){
	    ...
	    url: "/${appName}/manager/roleController/getAllRoleByPage",
	    method: 'post',
        ...
        columns: [
                ...
                },
                {
                    ...
                    formatter:function(value,row,index){
	                 var e = '<a href="#" class="btn btn-gmtx-define1" disabled="disabled" onclick="edit(\''+ row.roleCode +'\',\''+row.roleName+'\')">编辑</a> ';
	                 var d = '<a href="#" class="btn btn-gmtx-define1" disabled="disabled" onclick="delRole(\''+ row.roleCode +'\')">删除</a> ';
	                 var f = '<a href="#" class="btn btn-gmtx-define1"  onclick="accessShow(\''+ row.rolecode +'\')">权限设置</a> ';
                     return e+d+f; 
                  }
                }
            ]
      ...
      function addRole(){
		...
		$.ajax({
			url:'/${appName}/manager/roleController/addRole',
			type:'post',
			async:'true',
			cache:false,
	  ...
	  function delRole(rolecode){
	  	swal({
            ...
            if (isConfirm) {
                $.ajax({
					url:'/${appName}/manager/roleController/delRole',
					type:'post',
					async:'true',
					cache:false,
		...
		function upRole(){
			...
			$.ajax({
			url:'/${appName}/manager/roleController/upRole',
			type:'post',
			async:'true',
			cache:false,
			data:{roleCode:rcode_edit,roleName:roleName},
			dataType:'json',
			success: function(data){
					  if(data.isExist){
					  	 swal({
		...
		function saveAccess(){
			...
			$.ajax({
					url:'/${appName}/manager/roleController/bindModule',
					type:'post',
					async:'true',
					cache:false,
					data:{roleCode:roleCode_access,mids:mids},
					dataType:'json',
					success: function(data){
							  if(data.success){
							...
							<form method="post" class="form-horizontal" id="addform">
								<div class="control-group">
									...
									<div class="col-sm-8 controls">
										<input type="text" value="" class="form-control" name="roleName" id="roleName" />
									</div>
								</div>
								</br></br></br>
								<div class="control-group">
						...
						<div class="row">
							<form method="post" class="form-horizontal" id="upform">
								<div class="control-group">
									...
									<div class="col-sm-8 controls">
										<input type="text" value="" class="form-control" name="roleName" id="roleName_edit" />
									</div>
									</br></br></br>
									<div class="control-group">
										<div class="controls">
			...
			<div class="modal-dialog" style="width: 400px; height: 250px">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-gmtx-define1 center-block" onclick="saveAccess()">
							保存
						</button>
					</div>
				</div>
